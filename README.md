...
- **구글시트 연동 + 시트 탭별 분류**: "시트 탭 전부 불러오기" → 각 시트 탭이 주(week) 탭으로
  분류되고, 상단 탭 바로 주를 넘기며 편집한다. 새 주차 탭은 그 탭의 URL/gid를 "＋ 탭 추가"로 붙여 넣으면 됨
  (gid 하드코딩 없음). 각 주는 독립 — 한 주를 고쳐도 다른 주는 그대로. HTML 복사는 현재 보고 있는 주를 출력.
