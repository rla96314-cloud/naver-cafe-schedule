...
async function saveConfigToSheet(btn) {
  ...
  try {
    ...
    await fetch(url, { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify(configRows()) });
    ...
    btn.disabled = false;
    setTimeout(() => { btn.textContent = '설정을 시트에 저장 (전원 반영)'; btn.style.background = C.accent; }, 2000);
  }
}
function membersTSV() {
...
function settingsView() {
  ...
  if (_setTab === 'typo') {
    ...
    // (A) 자동 저장 — 웹앱 있으면 버튼 한 번으로 config 탭 갱신
    const saveBtn = el('button', { style: btnPrimary, onclick: () => saveConfigToSheet(saveBtn) }, '설정을 시트에 저장 (전원 반영)');
    wrap.append(section('설정을 시트에 저장 (버튼 한 번)', [
      ...
      el('div', { style: 'font-size:11.5px;color:var(--sub);line-height:1.6' },
        '한 번만 설치: 시트 → 확장 프로그램 → Apps Script에 apps-script-config-writer.gs 붙여넣고 "웹 앱(모든 사용자)"로 배포 → 나온 주소를 위에 붙여넣기. 이후 버튼만 누르면 config 탭이 갱신돼요.'),
    ]));
