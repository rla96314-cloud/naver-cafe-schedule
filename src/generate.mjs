...
export function generateScheduleHTML({ members = [], schedule = [], dates = {}, theme = {} } = {}) {
  const t = {
    ...
    cardHeight:100, nameFont:0 /* 0=자동(고정 내용이 다 보이는 최대) */, titleFont:18,
  ...
  /* ③④는 변동성 없는 데이터(멤버 목록·시간 형식) → 이번 주 최장 이름·최장 시간을 미리 알 수 있다.
     ...
     열 내용폭: (740 - 바깥패딩28)/7 - 셀패딩8 - 카드패딩16 ≈ 77px */
  const COL_CONTENT = Math.floor((740 - 28) / 7) - 8 - CARD_PAD * 2;
  ...
  const autoF = Math.floor((COL_CONTENT - ZONE_GAP) / (maxNameLen * 0.93 + maxTimeLen * 0.56 + 0.72));
  const nameF = (+t.nameFont > 0) ? +t.nameFont : Math.max(9, Math.min(40, autoF)); // ③④ 고정(축소 없음)
  const titleF = Math.max(8, Math.round((+t.titleFont || 18) * sc));                 // ⑤ (fontScale 적용)
  const pillW = Math.ceil(nameF * (maxTimeLen * 0.56 + 0.72)) + 2;                   // ④ 구역 = 딱 필요한 폭
  ...
  const card = (c, narrow) => {
    ...
    const headerZone =
      ...
      `<td style="padding:0 0 0 ${ZONE_GAP}px;vertical-align:top;width:${pillW}px"><div style="height:${nameZoneH}px;overflow:hidden;text-align:right;white-space:nowrap">${pill}</div></td>` +
      `</tr></table>`;
    ...
    const titleInner = c.title
      ...
      : '';
    /* ② 아바타 구역 — 제목 구역 오른쪽 정사각. 제목이 최소 60% 폭을 갖도록 폭 상한(40%). */
    const avS = Math.min(titleZoneH, Math.round(COL_CONTENT * 0.4));
    const avatar = img
      ? `<img src="${escapeAttr(img)}" alt="" style="width:${avS}px;height:${avS}px;object-fit:cover;${radius ? `border-radius:${Math.min(8, radius)}px;` : ''}display:block">`
      : '';
    const bodyZone = avatar
      ? `<table cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse;table-layout:fixed"><tr>` +
        ...
        `<td style="padding:0 0 0 ${ZONE_GAP}px;vertical-align:top;width:${avS + ZONE_GAP}px">${avatar}</td>` +
        `</tr></table>`
