...
export function generateScheduleHTML({ members = [], schedule = [], dates = {}, theme = {} } = {}) {
  ...
  const card = (c, narrow) => {
    ...
    if (narrow) {
      ...
      // 이름 + 시간을 한 줄 헤더로 고정(왼쪽 이름 / 오른쪽 알약), 제목은 그 아래.
      body =
        `<table cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse"><tr>` +
        `<td style="padding:0;text-align:left;vertical-align:middle">${link(nameHtml)}</td>` +
        `<td style="padding:0 0 0 6px;text-align:right;vertical-align:middle;width:1%;white-space:nowrap">${pill}</td>` +
        `</tr></table>${titleBlock}` +
